...
public class Course_tab_timetable_Teacher {
    ...
    //构造函数
    public Course_tab_timetable_Teacher(Teacher teacher, ArrayList<Course> myCourses) {
...
//        myCourses = (ArrayList<Course>) serverResponse.getData();
        ...
        commit.addEventHandler(MouseEvent.MOUSE_PRESSED, e-> {
            ...
            if(serverResponse_querySeme.isLastOperState()==true)
            {
                //重新打新表
                ...
                AnchorPane table_new = new AnchorPane();
                table_new = TableGenerate(myCourses_anotherSeme,sp,week);
                ...
                layout_f.setBody(new Label("      " + serverResponse_querySeme.getErrorMessage()));
                System.out.println(serverResponse_querySeme.getErrorMessage());
                JFXAlert<Void> alert_f = new JFXAlert<>((Stage) commit.getScene().getWindow());
